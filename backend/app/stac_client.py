...
async def buscar_imagens_stac(params: STACRequest):
    ...
    for item in items:
        ...
        # Aplica filtro de nuvens se necessário
        if params.filtrar_nuvens and (cloud_cover is None or cloud_cover > 10):
            continue

        assets = item.assets
        
        # Adicionando a verificação dos assets
        print(f"Assets disponíveis para o item {item.id}: {assets.keys()}")  # Adicionando o log para assets disponíveis
        ...
        # Caso o cmask não esteja disponível, ele será None, e o log ajudará a verificar isso
        if cmask:
            print(f"CMASK disponível para o item {item.id}: {cmask.href}")
        else:
            print(f"CMASK NÃO disponível para o item {item.id}")
