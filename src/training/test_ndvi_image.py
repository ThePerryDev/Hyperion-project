...
image_path = "images/CBERS_4_AWFI_20240930_159_123.tif"
model_path = "models/final_model_1.pth"
output_dir = "output"
os.makedirs(output_dir, exist_ok=True)

output_png = os.path.join(output_dir, "CBERS_4_AWFI_20240930_159_123.png")
output_tif = os.path.join(output_dir, "CBERS_4_AWFI_20240930_159_123.tif")
...
def print_ndvi_distribution(ndvi_array):
    print(f"📈 NDVI - min: {ndvi_array.min():.4f}, max: {ndvi_array.max():.4f}")
    print("📊 Faixas de NDVI:")
    bins = [(-1.0, 0.1), (0.1, 0.2), (0.2, 0.3), (0.3, 1.0)]
    total_pixels = ndvi_array.size
    for idx, (low, high) in enumerate(bins):
        count = np.logical_and(ndvi_array >= low, ndvi_array < high).sum()
        print(f"  - Classe {idx} ({low:.1f} a {high:.1f}): {count} pixels ({(count/total_pixels)*100:.2f}%)")
...
# ➕ Imprimir distribuição NDVI
print_ndvi_distribution(ndvi_array)
...
for cls, cnt in zip(unique, counts):
    print(f"  - Classe {cls}: {cnt} pixels ({cnt / total * 100:.2f}%)")
